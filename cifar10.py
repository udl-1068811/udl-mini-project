...
from models import Vanilla_NN, MFVI_NN, DCGAN_CIFAR10_Generator, DCGAN_CIFAR10_Discriminator
from coreset import RandomCoreset, KCenterCoreset
from pipeline_ddim import DDIMPipeline
from cifar_classifier import cifar10
...
def cifar10_dataset():
    ...
    cifar_transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)), transforms.Lambda(lambda x: x.view(-1))])

    train_datasets = CIFAR10(root="~/.torch/data/mnist", train=True, download=True, transform=cifar_transform)
    test_datasets = CIFAR10(root="~/.torch/data/mnist", train=False, download=True, transform=cifar_transform)

    gan_cifar_transform = transforms.Compose([transforms.Resize(64), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    gan_train_dataset =  CIFAR10(root="~/.torch/data/mnist", train=True, download=True, transform=gan_cifar_transform)
    ...
    return train_datasets, train_task_ids, test_datasets, test_task_ids, gan_train_dataset, binarize_y
...
def run_vcl_experiment(opt, train_datasets, train_task_ids, test_datasets, test_task_ids, device):
    ...
    for task_id in range(opt.n_tasks):
        ...
        # Save model
        os.makedirs("mfvi-models/cifar10", exist_ok=True)
        if coreset is None:
            torch.save(mfvi_model.state_dict(), f"mfvi-models/cifar10/vcl_task_{task_id}.pt")
        else:
            torch.save(mfvi_model.state_dict(), f"mfvi-models/cifar10/vcl_coreset_{opt.coreset_alg}_task_{task_id}.pt")
    ...
    if coreset is None:
        np.save(f"results/cifar10/vcl.npy", mean_accuracies)
    else:
        np.save(f"results/cifar10/vcl_coreset_{opt.coreset_alg}.npy", mean_accuracies)

def train_gan(train_dataloader, epochs, device):
    generator = DCGAN_CIFAR10_Generator().to(device)
    discriminator = DCGAN_CIFAR10_Discriminator().to(device)
...
def run_vgr_experiment(opt, train_datasets, train_task_ids, test_datasets, test_task_ids, gan_train_datasets, y_transform, device):
    print("Running VGR (GAN) experiment")
    
    # Create directory to save GANs
    os.makedirs("vgr-gan/cifar10", exist_ok=True)
    ...
    for task_id in range(opt.n_tasks):
        ...
        if task_id != 0:
            ...
            # Instantiate a separate dataloader for each task
            train_task_indices = torch.where(train_task_ids == task_id)[0]
            gan_task_data = Subset(gan_train_datasets, train_task_indices)
            ...
            for _task in range(task_id):
                for cls in range(2):
                    generator = DCGAN_CIFAR10_Generator()
                    generator.load_state_dict(torch.load(f"vgr-gan/cifar10/generator_task_{_task}_class_{_task * 2 + cls}.pt"))
                    generator = generator.to(device)
                    generator.eval()
                    z = torch.randn((1000, 100, 1, 1)).to(device)
                    generated_data = generator(z)
                    generated_data = Resize((32, 32))(generated_data)
                    generated_data = generated_data.reshape(1000, 3*32*32).detach().cpu()
                    generated_data_labels = torch.full((1000,), _task * 2 + cls)
            ...
            for cls in range(2):
                ...
                class_data = Subset(gan_task_data, class_indices)
                ...
                torch.save(generator.state_dict(), f"vgr-gan/cifar10/generator_task_{task_id}_class_{task_id * 2 + cls}.pt")
            
            # Save model
            torch.save(mfvi_model.state_dict(), f"mfvi-models/cifar10/vgr_gan_task_{task_id}.pt")
    ...
    # Save results
    os.makedirs("results/cifar10", exist_ok=True)
    np.save(f"results/cifar10/vgr_gan.npy", mean_accuracies)
...
def run_vgr_dm_experiment(opt, train_datasets, train_task_ids, test_datasets, test_task_ids, y_transform, device):
    ...
    cifar_classifier = cifar10(128, pretrained='log/cifar10/best-135.pth').eval()  
        ...
        if task_id != 0:
            ...
            for _task in range(task_id):
                for cls in range(2):
                    ...
                    ddim_pipeline.set_progress_bar_config(disable=False)
                    generated_data = []
                    for _ in range(20):
                        sample = ddim_pipeline(batch_size=1000, num_inference_steps=opt.num_inference_steps)
                        class_cifar = cifar_classifier(sample).data.max(1)[1]
                        sample = sample[class_cifar == _task * 2 + cls]
                        sample = sample.view(sample.size(0), -1)
                        generated_data.append(sample.detach().cpu())
                    generated_data = torch.cat(generated_data, dim=0)
                    print(f"Number of samples from class {_task * 2 + cls}: {generated_data.shape[0]}")
                    generated_data_labels = torch.full((generated_data.shape[0],), _task * 2 + cls)
                    generated_dataset.append(TensorDataset(generated_data, generated_data_labels))
                    del ddim_pipeline
                    torch.cuda.empty_cache()
            ...
            # Save model
            torch.save(mfvi_model.state_dict(), f"mfvi-models/cifar10/vgr_ddpm_task_{task_id}.pt")
    ...
    # Save results
    os.makedirs("results/cifar10", exist_ok=True)
    np.save(f"results/cifar10/vgr_ddpm.npy", mean_accuracies)
...
def main():
    ...
    parser.add_argument('--coreset_size', type=int, default=100, help='Size of the coreset.')
    ...
    parser.add_argument('--num_inference_steps', type=int, default=25, help='Number of inference steps for diffusion mode.')
    ...
    # Dataset
    train_datasets, train_task_ids, test_datasets, test_task_ids, gan_train_dataset, y_transform = cifar10_dataset()
    ...
    if opt.experiment == "all":
        ...
        # VGR (GAN)
        run_vgr_experiment(opt, train_datasets, train_task_ids, test_datasets, test_task_ids, gan_train_dataset, y_transform, device)
